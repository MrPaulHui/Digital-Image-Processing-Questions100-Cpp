...
- 直方图均衡化
  ...
  https://blog.csdn.net/spongebob1234/article/details/77778709 （这个牛逼）

  涉及到概率论求概率密度函数，主要证明均衡化变换后呈均匀分布。

  推导过程：

  1. 设原始图像的灰度为x，均衡化后图像的灰度为y，则均衡化对应的函数为$y=T(x)$，x和y的取值范围均为[0,L-1]，一般L为256
  ...
  2. 设原始图像灰度分布为$F_X(x)$，概率密度函数为$f_X(x)$，均衡化后图像灰度分布为$F_Y(y)$，概率密度函数为$f_Y(y)$。这里可以发现，F就是累积直方图，f就是直方图（归一化后的）

     其中，
     $$
     f_X(x)=\frac{x_k}{MN},\ \ k=0,1,...,L-1
     $$
     MN为图像长宽，$x_i$表示第i个灰度所包括的像素点总数

     我们的目标是对直方图进行均衡化，也就是使变换后的图像灰度呈均匀分布，则有
     $$
     f_Y(y)=\frac{1}{L-1}
     $$
  ...
  3. 分布之间的关系推导：
     ...
     两边同时对y求导，得到
     $$
     f_Y(y)=f_X(T^{-1}(y))\frac{d(T^{-1}(y))}{dy}=f_X(x)\frac{dx}{dy}
     $$
     代入第2步和第1步得到的条件，有
     $$
     \frac{1}{L-1}=\frac{x_k}{MN}\frac{dx}{d(T(x))}\\
     \frac{d(T(x))}{dx}=\frac{(L-1)x_k}{MN}
     $$
     两边对x求积分，又x为离散，直接求和即可，
     $$
     y_k=\frac{L-1}{MN}\sum_{i=0}^k x_i
     $$
...
- 优点
  1. HOG是边缘（梯度）的结构特征，可以描述图像的形状信息
  2. 方向上的量化可以一定程度减少旋转的影响
  3. 局部区域进行直方图归一化，减小了光照的影响
  4. 分块分单元的处理方法，使图像局部像素点之间的关系可以得到表征
- 缺点
  1. 速度慢，实时性差
  2. 很难处理遮挡问题
  3. 由于使用的是梯度信息，对噪点敏感
...
https://www.cnblogs.com/emouse/p/3611256.html 理论推导
...
- $\gamma$影响椭圆度，为1的时候就是圆形，越大越扁
- $\sigma$影响尺度，即滤波器的大小，越大越大
- A角度影响方向，具体体现在计算$x^{'}$和$y^{'}$的公式，为乘A角度的旋转矩阵；注意公式里的旋转角即A是以x轴和y轴负轴为坐标系的。

关于Gabor能提取边缘的我自己的理解：

Gabor滤波就是短时（加窗）傅里叶变换，作用是提取局部的频域信息，（傅里叶变换是提取全局的频域信息），对于图像来说，局部的高频就是这个局部区域的边缘，低频就是平坦区域，所以有边缘提取的作用。而传统的傅里叶变换提取了全局的频域，失去了位置信息，所以没有检测边缘的能力。

Gabor提取特征：多个角度滤波得到的边缘叠加，据说和CNN很类似。。。
...
参考：https://blog.csdn.net/lyl771857509/article/details/79675137

#### step 1. 构建高斯金字塔

1. 对图像做不同尺度$\sigma$的高斯模糊
2. 对图像进行降采样（尺寸缩小）

具体做法步骤是：

每一组（octave）是一系列尺寸相同，高斯模糊程度不同的图像，每组图像数记为N；

每一组的倒数第三张图像作为下一组的基底，下一组其他图像由这个基底进行不同尺度的高斯模糊得到（第一组的基底是原图）；

以此类推，生成所有的组，构成高斯金字塔。

#### step 2. DoG空间极值点检测

1. 构建高斯差分金字塔，即每一组的一系列图像进行上一层减下一层，得到N-1张差分图，每张差分图的像素值就是在这个尺度下的DoG值（更准确说是散度值，参见DoG原理）。
2. 对一张差分图上的一个像素点，取这个像素点在这个差分图8邻域点、以及上层差分图相邻的9个点、下层差分图相邻的9个点，共计26个点，比较该像素点是否比这26点都大或都小，即是否为极值点。初步得到极值点的候选位置。（N-1张差分图，最上面的和最下面的是没法比较的，所有只有N-3张差分图有效，若要保证每组有S个不同尺度的差分图，则需要N=S+3）

每个初步候选的极值点包括三维信息$(x,y,\sigma)$，即位置和尺度，（个人理解，这里的位置x，y应该归一化到原图尺寸上）

#### step 3. 关键点精确定位：通过对极值点精确定位、删除DoG值过小的极值点、消除边缘响应

1. 通过初步候选的极值点及其对应的DoG值，拟合DoG函数曲线，求出精确的极值点和初步候选点相对其的偏移量。DoG函数表示为$D(X)$，其中$X\in R^3$，$D(X)\in R$
2. $|D(X)|$值过小的极值点，容易受到噪声的影响，设置一个经验阈值，删除过小的极值点
3. 消除边缘响应的极值点（好像就是边缘点，具体做法有些复杂，关于曲率和hessian矩阵特征值，还需要再看）
4. 通过第2,3步删除后，保留下来的极值点，加上第一步计算出的偏移量，得到最终精确的关键点
