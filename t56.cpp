//模式匹配——Normalization Cross Correlation
...
cv::Point Pattern_Match_NCC(cv::Mat img, cv::Mat T, bool disp=true){
    int height = img.rows;
    int width = img.cols;
    int Th = T.rows;
    int Tw = T.cols;
    ...
    for(int y=0;y<height-Th;y++){
        for(int x=0;x<width-Tw;x++){
            ...
            for(int dy=0;dy<Th;dy++){
                for(int dx=0;dx<Tw;dx++){
                    for(int c=0;c<channel;c++){
                        sum += img.at<cv::Vec3b>(y+dy,x+dx)[c]*T.at<cv::Vec3b>(dy,dx)[c];
...
int main(){
    ...
    cv::Point best_p = Pattern_Match_NCC(img, T);
