//模式匹配——Sum of Absolute Differences mae
...
cv::Point Pattern_Match_SAD(cv::Mat img, cv::Mat T, bool disp=true){
    int height = img.rows;
    int width = img.cols;
    int Th = T.rows;
    int Tw = T.cols;
            ...
            for(int dy=0;dy<Th;dy++){
                for(int dx=0;dx<Tw;dx++){
                    for(int c=0;c<channel;c++){
                        sum += fabs(img.at<cv::Vec3b>(y+dy,x+dx)[c]-T.at<cv::Vec3b>(dy,dx)[c]); //MAE
                    }
                }
            }
            if(sum<min){
...
int main(){
    ...
    cv::Point best_p = Pattern_Match_SAD(img, T);
