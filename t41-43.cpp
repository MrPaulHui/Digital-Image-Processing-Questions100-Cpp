...
cv::Mat Canny(cv::Mat img, double high_thresh=100, double low_thresh=20, bool disp_middle=false){
    ...
    gray = Guassian_Filter(gray, 1.4, 5);
    ...
    cv::Mat grad_row = Conv_Gray(gray, sobel_kernel_row);
    cv::Mat grad_col = Conv_Gray(gray, sobel_kernel_col);
    //step 4. get grad magnitude and direction
    //cv::Mat grad_magnitude = Clip(out_row + out_col); //这一步是近似运算
    cv::Mat grad_magnitude = cv::Mat::zeros(height, width, CV_8UC1); //这一步数据类型存疑啊，uchar是不是精度太低了，保存的可是梯度啊，包括conv函数里的似乎也有这个问题
    cv::Mat grad_direction = cv::Mat::zeros(height, width, CV_8UC1);
    double grad_y, grad_x, grad_m, grad_d, angle;
    for(int i=0;i<height;i++){
        for(int j=0;j<width;j++){
            grad_y = (double)grad_row.at<uchar>(i,j);
            grad_x = (double)grad_col.at<uchar>(i,j);
            //step 4.1. get magnitude
            grad_m = sqrt(grad_y*grad_y+grad_x*grad_x);
            grad_magnitude.at<uchar>(i,j) = (uchar)grad_m;
